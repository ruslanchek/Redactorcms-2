...
<div class="collapsable_fieldlist_item_tools">
    <fieldset>
    	...
    	<table cellpadding="0" cellspacing="0" border="0" class="item_tools_table">
    		...
    		<tr>
    			...
    			<td class="content_col">
    				{if $new}
			            <input class="textfield" type="text" value="{$main->getText('sections', 'form_editor_tools_multifile')}" name="label**__**{$new}" id="label_new_{$item.new}" />
			        {else}
			...
			<tr id="custom_{if $new}{$new}{else}{$item.id}{/if}" style="display: none;">
    			...
    			<td class="content_col">
    				<input autocomplete="off" type="hidden" value="{$item.options_custom}" name="options_custom**__**{if $new}{$new}{else}{$item.id}{/if}" id="options_custom_{if $new}{$new}{else}{$item.id}{/if}" />
        ...
        <input type="hidden" value="multifile" name="type**__**{if $new}{$new}{else}{$item.id}{/if}" />
