...
<div class="collapsable_fieldlist_item_tools">
    <fieldset>
    	<table cellpadding="0" cellspacing="0" border="0" class="item_tools_table">
    		...
    		<tr>
    			...
    			<td class="content_col">
    				{if $new}
			            <input class="textfield" type="text" value="{$main->getText('sections', 'form_editor_tools_multiselect')}" name="label**__**{$new}" id="label_new_{$item.new}" />
			        {else}
    		...
    		<tr id="defaults_{if $new}{$new}{else}{$item.id}{/if}" style="display: none;">
    			...
    			<td class="content_col">
    				<select class="select" multiple="multiple" name="default**__**{if $new}{$new}{else}{$item.id}{/if}[]" id="default_{if $new}{$new}{else}{$item.id}{/if}">
        ...
        <input type="hidden" value="multiselect" name="type**__**{if $new}{$new}{else}{$item.id}{/if}" />
        <input type="hidden" value="{$item.sort}" name="sort**__**{if $new}{$new}{else}{$item.id}{/if}" rel="sort" />
    </fieldset>
</div>

<script type="text/javascript">
	fieldsEditor.selectItemInit('select_{if $new}{$new}{else}{$item.id}{/if}', '{$item.options_source}', '{$item.default}', 'multiselect');
</script>