...
<div class="collapsable_fieldlist_item_tools">
    <fieldset>
    	<table cellpadding="0" cellspacing="0" border="0" class="item_tools_table">
    		...
    		<tr>
    			...
    			<td class="content_col">
    				{if $new}
			            <input class="textfield" type="text" value="{$main->getText('sections', 'form_editor_tools_param')}" name="label**__**{$new}" id="label_new_{$item.new}" />
			        {else}
            ...
            <tr>
    			<td class="label_col">
    				<label for="prefix_{if $new}{$new}{else}{$item.id}{/if}">{$main->getText('sections', 'form_edit_item_prefix')}</label>
    			</td>
    			<td class="content_col">
    				<input class="textfield" type="text" value="{$item.prefix}" name="prefix**__**{if $new}{$new}{else}{$item.id}{/if}" id="prefix_{if $new}{$new}{else}{$item.id}{/if}" />
    			</td>
    		</tr>

            <tr>
    			<td class="label_col">
    				<label for="suffix_{if $new}{$new}{else}{$item.id}{/if}">{$main->getText('sections', 'form_edit_item_suffix')}</label>
    			</td>
    			<td class="content_col">
    				<input class="textfield" type="text" value="{$item.suffix}" name="suffix**__**{if $new}{$new}{else}{$item.id}{/if}" id="suffix_{if $new}{$new}{else}{$item.id}{/if}" />
    			</td>
    		</tr>
			...
			<tr>
    			...
    			<td class="content_col">
					...
					<input type="hidden" name="email**__**{if $new}{$new}{else}{$item.id}{/if}" value="0" id="email_hidden_{if $new}{$new}{else}{$item.id}{/if}" />
        ...
        <input type="hidden" value="param" name="type**__**{if $new}{$new}{else}{$item.id}{/if}" />
