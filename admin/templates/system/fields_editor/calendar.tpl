{if !$default}
<a href="javascript:void(0)" class="icon_action icon_delete_instance" onclick="fieldsEditor.deleteField('{if $new}{$new}{else}{$item.id}{/if}', '{$main->getText('sections', 'form_editor_delete_field_confirm')}', '{if $new}0{else}{$main->item_data.id}{/if}')" title="{$main->getText('sections', 'form_editor_delete_field')}, {if $new}0{else}1{/if}"></a>
...
<div class="collapsable_fieldlist_item_tools">
    <fieldset>
    	...
    	<table cellpadding="0" cellspacing="0" border="0" class="item_tools_table">
    		...
    		<tr>
    			...
    			<td class="content_col">
    				{if $new}
			            <input class="textfield" type="text" value="{$main->getText('sections', 'form_editor_tools_calendar')}" name="label**__**{$new}" id="label_new_{$item.new}" />
			        {else}
        ...
        <input type="hidden" value="calendar" name="type**__**{if $new}{$new}{else}{$item.id}{/if}" />
