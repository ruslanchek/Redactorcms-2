    	...
    	<table cellpadding="0" cellspacing="0" border="0" class="item_tools_table">
    		...
    		<tr>
    			...
    			<td class="content_col">
    				{if $new}
			            <input class="textfield" type="text" value="{$main->getText('sections', 'form_editor_tools_color_picker')}" name="label**__**{$new}" id="label_new_{$item.new}" />
			        {else}
			...
			<tr>
    			<td class="label_col">
    				<label for="randomize_{if $new}{$new}{else}{$item.id}{/if}">{$main->getText('sections', 'form_edit_randomize')}</label>
    			</td>
    			<td class="content_col">
    				<input type="checkbox" value="1" name="randomize**__**{if $new}{$new}{else}{$item.id}{/if}" {if $item.randomize == '1'}checked="checked"{/if} id="randomize_{if $new}{$new}{else}{$item.id}{/if}" />
    			</td>
    		</tr>
			
    	</table>
        
        <input type="hidden" value="color_picker" name="type**__**{if $new}{$new}{else}{$item.id}{/if}" />
