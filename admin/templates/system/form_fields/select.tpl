<div class="item_block" id="select_{if !$item.slave_of}{$item.name}{else}{$item.meta}{/if}" {if $item.slave_of}style="position: absolute; left: -10000px"{/if}>
    ...
    <select class="select" data-name="{if !$item.slave_of}{$item.name}{else}{$item.meta}{/if}" {if $item.slave_of}data-slave="true"{/if} {if !$item.slave_of}name="{$item.name}"{/if} id="{if !$item.slave_of}{$item.name}{else}{$item.meta}{/if}" tabindex="{$index+1}">
    ...
    {if $item.link}
        {if $item.value > 0}<span class="select-action-btn"> <a href="#" data-action_header="Редактирование объекта" class="ajax_viewport_link" data-src="{$item.link}{$item.value}"><i class="icon-edit"></i>Редактировать</a></span>{/if}
    {/if}

    {if $item.create_link}
        <span class="select-action-btn"> <a href="#" data-action_header="Создание, привязка, а затем редактирование объекта" class="ajax_viewport_link" data-src="{$item.create_link}"><i class="icon-plus-squared"></i>Создать</a></span>
    {/if}
</div>
