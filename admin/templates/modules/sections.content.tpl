<div class="list_actions_toolbar">
    <div class="list_menu_buttons">
        <a class="button" href="/admin/?option=sections&suboption=content&action=create&id={$main->item_data.id}" tabindex="1">
            <span>
                <img class="button_icon icon_action icon_add_instance" src="/admin/img/frames/e.gif" />
                <b>{$main->getText('sections', 'create_new_item')}</b>
            </span>
        </a>
        <a class="button multiaction_button" href="javascript:void(0)" onclick="list.multiOperation('{$main->getText('list', 'confirm_multiple_delete')}', 'del', '/admin/?ajax=true&option=sections&suboption=content&id={$main->item_data.id}&action=multiple')">
            <span>
        ...
        <a rel="show" class="button multiaction_button" href="javascript:void(0)" onclick="list.multiOperation('{$main->getText('list', 'confirm_multiple_show')}', 'show', '/admin/?ajax=true&option=sections&suboption=content&id={$main->item_data.id}&action=multiple')">
            <span>
                <img class="button_icon icon_action icon_hide_instance" src="/admin/img/frames/e.gif" />
                {$main->getText('sections', 'show_selected_items')}
            </span>
        </a>
        <a rel="hide" class="button multiaction_button" href="javascript:void(0)" onclick="list.multiOperation('{$main->getText('list', 'confirm_multiple_hide')}', 'hide', '/admin/?ajax=true&option=sections&suboption=content&id={$main->item_data.id}&action=multiple')">
            <span>
        ...
        <a class="button right_button" href="javascript:void(0)" onclick="list.multiOperation(false, 'reorder', '/admin/?ajax=true&option=sections&suboption=content&id={$main->item_data.id}&action=multiple')">
            <span>
        ...
        <a class="button" href="javascript:void(0)" onclick="list.ie.open('{$main->import_link}', '{$main->export_link}')">
            <span>
                Импорт-экспорт
            </span>
        </a>
    ...
    <div class="stripe_form">
        <form action="/admin/" method="GET">
            ...
            <input type="hidden" name="id" value="{$main->item_data.id}" />
...
<script type="text/javascript">
    ...
    $('#content_search').ready(function(){
        autocompleteSearch($('#content_search'), '/admin/?ajax=true&action=autocomplete_search&option=sections&suboption=content&id={/literal}{$main->item_data.id}{literal}');
    });
